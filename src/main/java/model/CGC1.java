...
//
    ...
    private ArrayList<Cord> checkAdj(Tile[][] bookshelf, ArrayList<Cord> listOfCords, Cord cord){
           ...
           for (int counter = 0; counter < listOfCords.size() || listOfCords.size()==0; counter++) {
                ...
                if(i>0)
                    if(bookshelf[i][j].getType() == bookshelf[i-1][j].getType() && !(bookshelf[i][j].getType().equals(Type.NOTHING))) {
                ...
                if(j<4)
                    if(bookshelf[i][j].getType() ==bookshelf[i][j+1].getType() && !(bookshelf[i][j].getType().equals(Type.NOTHING))) {
                        if(!(listOfCords.contains(cord))) {
                            cord.setCords(i, j);
                            listOfCords.add(cord);
                            }
        ...
        public boolean compareRule(Bookshelf bks, int id){
           ...
           for(int i=0; i<5; i++)
               for(int j=0;j<5;j++) {
                   found=false;
                   diff = true;
                   for (int counter = 0; counter < listOfCords.size() && !(listOfCords.isEmpty()) && !bookshelf[i][j].getType().equals(Type.NOTHING) ; counter++) {
                       if(listOfCords.get(counter).getRowCord() == i && listOfCords.get(counter).getColCord() ==j && bookshelf[i][j].getType().equals(Type.NOTHING)) { //ALERT!
                           diff = false;
                           break;
                       }
                   }
                   ...
                   for (int counter = 0; counter < listOfCords.size() && !(listOfCords.isEmpty()) ; counter++) {
                       ...
                       bookshelf[x][y].setType(Type.NOTHING);
                       found=true;
                   }
                   if(found)
                       score++;
